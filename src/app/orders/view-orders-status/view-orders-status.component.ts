...
export class ViewOrdersStatusComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    if (localStorage.getItem('sellerId')) {
      ...
      this.ordersService.getOrderStatus(id, salesman.id).subscribe((data: any) => {
        this.ordersData = data;
        this.dataSource.data = this.ordersData;
        console.log(salesman.id);
        // error => this.error = error
      });
    }
    if (localStorage.getItem('sellerIdMaster')) {
      const salesman = JSON.parse(localStorage.getItem('sellerIdMaster'));
      this.ordersService.getOrderStatus(id, salesman).subscribe((data: any) => {
        this.ordersData = data;
        this.dataSource.data = this.ordersData;
        // error => this.error = error
      });
