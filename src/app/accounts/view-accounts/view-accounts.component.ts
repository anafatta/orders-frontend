...
export class ViewAccountsComponent implements OnInit {
  ...
  accountData = null;
  sellerData: Seller[];
  displayedColumns: string[] = ['estado', 'codfac', 'nom', 'count', 'saldo', 'button'];
  ...
  ngOnInit() {
    ...
    if (localStorage.getItem('sellerId')) {
      ...
      this.accountService.get_Ctacli_Det1(salesman.id).subscribe((data: Det0[]) => {
    ...
    if (localStorage.getItem('sellerIdMaster')) {
      ...
      this.accountService.get_Ctacli_Det1(salesman).subscribe((data: Det0[]) => {
        this.accountData = data;
        this.dataSource.data = this.accountData;
      });
    }
